...
-- Event to open racing app from dynamic menu
RegisterNetEvent('cw-racingapp:client:openRacingApp', function()
    if OpenUi then
        OpenUi()
    end
end)

-- Event to try client export
RegisterNetEvent('cw-racingapp:client:tryOpenExport', function()
...
-- Test command to open UI directly from client
RegisterCommand("testui", function()
    TriggerEvent('chat:addMessage', {
        color = {0, 255, 0},
        multiline = true,
        args = {"[TEST]", "Abrindo UI diretamente..."}
    })
    OpenUi()
end, false)
...
local function joinRace(raceId)
    ...
    if PlayerIsInVehicle and IsDriver(vehicle) then
        ...
        -- Don't return false, allow joining without vehicle
    end
    ...
    if currentRace == nil then
        ...
        currentRace.PlayerVehicleEntity = vehicle
        
        -- Get race data for teleport
        local raceData = cwCallback.await('cw-racingapp:server:getRaceData', raceId)
        if raceData and raceData.Checkpoints then
            -- Teleport immediately before joining
            teleportToRaceStart(raceData.Checkpoints)
        end
...
-- NUI Callback to play sounds
RegisterNUICallback('playSound', function(data, cb)
    if data.sound then
        local soundPath = 'race_notification.wav'
        -- Play sound using FiveM's audio system
        SendNUIMessage({
            type = 'playAudio',
            sound = soundPath
        })
...
RegisterNUICallback('playCustomSound', function(data, cb)
    if data.soundFile then
        -- Try to play the custom sound file
        PlaySoundFrontend(-1, data.soundFile, "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 1)
    end
    cb(true)
end)

-- Handle NUI messages for playing audio files
RegisterNUICallback('playCustomSound', function(data, cb)
    if data.soundFile then
...
RegisterNUICallback('playAudioFile', function(data, cb)
    if data.file then
        -- This will be handled by the web interface
    end
    cb(true)
