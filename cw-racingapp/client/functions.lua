...
-- Update racer data from server
RegisterNetEvent('cw-racingapp:client:updateRacerData', function(racerData)
    MyRacerNames = racerData
    
    if racerData and racerData[1] then
        local racer = racerData[1]
        CurrentName = racer.racername
        CurrentAuth = racer.auth
        CurrentRanking = racer.ranking or 0
        CurrentCrypto = racer.crypto or 0
        CurrentCrew = racer.crew or ""
...
function NotifyHandler(message, type)
    ...
    TriggerEvent('Notify', "RACING", message, theme, 5000)
